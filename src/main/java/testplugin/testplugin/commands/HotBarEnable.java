...
public class HotBarEnable implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String [] args) {
        ...
        if (HotbarSetUtil.getSet(pl.getUniqueId().toString())) {
            Bukkit.getLogger().info(args[0] + "already has hotbarlock enabled");
        }
        else {
            HotbarSetUtil.setSet(pl.getUniqueId().toString());
            Bukkit.getLogger().info("Hotbarlock enabled for " + args[0]);
        }
        return true;
    }
}
